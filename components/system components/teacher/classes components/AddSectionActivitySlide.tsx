...
import { createActivitySlide } from "@/backend/activitslides";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { toast } from "react-toastify";
const AddSectionActivitySlide = ({ activityId }: any) => {
  ...
  const { mutate: createSlideMutate, isPending } = useMutation({
    mutationFn: createActivitySlide,

    onSuccess: () => {
      toast.success("Slide succesfully created");

      queryClient.invalidateQueries({ queryKey: ["activityslides"] });
    ...
    <div>
      <Dialog>
        <DialogTrigger asChild>
          <Button className="bg-linear-to-r from-[#FF5B5B] to-[#F04886] text-white flex items-center gap-2 shadow shadow-black/30 mt-5  ">
...
export default AddSectionActivitySlide;
